...
function main () {
	...
	function oau_initButton() {
		...
		// remove if exists
		$('#open-all-unread-split-button').remove();

		// create button

		$('<span id="open-all-unread-split-button">'
			+ '<div id="open-all-unread-button" tabindex="0" class="goog-inline-block jfk-button jfk-button-standard jfk-button-collapse-right" role="button" style="-moz-user-select: none;">'
				+ $('#open-all-unread-options .goog-option-selected').text()
			+ '</div>'
		...
		$('#open-all-unread-button').hover(function() { $(this).addClass('jfk-button-hover'); }, function() { $(this).removeClass('jfk-button-hover'); });
		$('#open-all-unread-options-dropdown').hover( function() { $(this).addClass('goog-flat-menu-button-hover'); }, function() { $(this).removeClass('goog-flat-menu-button-hover'); });
		// hide menu if click anywhere
		$('body').click(function(eventObject) { if ($(eventObject.target).closest('#open-all-unread-options-dropdown').length == 0) $('#open-all-unread-options').hide(); });
		// hide menu on escape
		$(document).keydown( function(eventObject){ if(eventObject.which == 27) $('#open-all-unread-options').hide(); });
		//
		$('#open-all-unread-options-dropdown').click(function() {
			$('#open-all-unread-options').toggle();
			$('#open-all-unread-options').position({
				my: 'left top',
				at: 'left bottom',
				of: $('#open-all-unread-split-button'),
			});
		});
	}
	...
	function oau_initOptions() {
		...
		$('#open-all-unread-options').remove();
		$('<div id="open-all-unread-options" class="goog-menu goog-menu-vertical" role="menu" aria-haspopup="true" aria-activedescendant=""></div>').appendTo('body').hide();
		oau_text_p1 = '<div class="goog-menuitem goog-option" role="menuitem" style="-moz-user-select: none;"><div class="goog-menuitem-content">';
		oau_text_p3 = '</div></div>';
		$.each(oau_options, function(index, value) {
			$(oau_text_p1 + value[1] + oau_text_p3)
				.attr('rel', value[0])
				.click(function() { oau_setOpenCount($(this).attr('rel')); })
				.hover(function() { $(this).addClass('goog-menuitem-highlight'); },function() { $(this).removeClass('goog-menuitem-highlight'); })
				.appendTo('#open-all-unread-options')
				;
		});
		$('#open-all-unread-options .goog-menuitem[rel=' + oau_openCount.toString() + ']').addClass('goog-option-selected');
	...
	function oau_openAllUnread () {
		...
		var oau_openCount = oau_getOpenCount();
		var entries = $('div.entry:not(.read) a.entry-original, div.entry:not(.read) a.entry-title-link').slice(0, oau_openCount);
		...
		entries.each(function(index, Element){
			...
			// Mark as readed
			oau_fireMouseEvent($(Element).closest('div.collapsed, div.card').get(0), 'click');
			//$(Element).closest('div.collapsed, div.card').click();
		});
